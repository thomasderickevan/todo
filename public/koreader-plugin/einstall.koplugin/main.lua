--[[
    einstall Package Manager for KOReader
    Native e-ink package manager & installer for jailbroken Kindle and Kobo devices.
    Copyright (c) 2026 ENDEAVOR OS // einstall Ecosystem
]]--
...
local EInstall = WidgetContainer:extend{
    name = "einstall",
    ...
    active_repos = {
        ...
        "https://ederick.vercel.app/einstall.json"
    }
}

function EInstall:init()
...
function EInstall:addToMainMenu(menu_items)
    menu_items.einstall = {
        text = _("einstall Package Manager"),
        sub_item = {
            {
...
function EInstall:fetchUrl(url)
    local response_body = {}
    ...
    local res, code, response_headers = client.request{
        ...
        method = "GET",
        headers = {
            ["User-Agent"] = "KOReader-einstall/2.0.0 (Kindle Linux)",
    ...
    if code == 200 or code == "200" then
        ...
        if url:sub(1, 5) == "https" then
            ...
            local fb_res, fb_code = http.request{
                url = fallback_url,
                method = "GET",
                headers = { ["User-Agent"] = "KOReader-einstall/2.0.0" },
...
function EInstall:fetchAndShowCatalog()
    local info = InfoMessage:new{
        text = _("Connecting to einstall Repositories over Wi-Fi...\nPlease ensure Wi-Fi is connected."),
    }
    UIManager:show(info)
    ...
    local catalog_menu = Menu:new{
        title = _("einstall // Available Plugins (") .. tostring(#pkg_list) .. _(")"),
...
function EInstall:showPackageDialog(pkg)
    local is_installed = self:isPluginInstalled(pkg.id)
...
function EInstall:showInstalledMenu()
    local installed_items = {}

    for file in lfs.dir(self.plugins_dir) do
        if file:match("%.koplugin$") and file ~= "einstall.koplugin" and file ~= "zenpm.koplugin" then
...
function EInstall:confirmUninstall(folder_name, clean_name)
    UIManager:show(ConfirmBox:new{
...
function EInstall:showRepoSettings()
    local repo_items = {}
    ...
    local menu = Menu:new{
        title = _("einstall // Repository Taps"),
...
function EInstall:promptAddRepo()
    local dialog
...
function EInstall:reloadPlugins()
    pcall(function()
